...
module.exports = class ModerationMuteCommand extends Command {
	constructor(client) {
		super(client, {
			...
			group: 'moderasyon',
			memberName: 'konuştur',
			description: 'İstediğiniz kişinin susturmasını açar',
			guildOnly: true,
			throttling: {
				usages: 2,
				duration: 3
	...
	async run(msg, args) {
		...
		if (!msg.guild.member(user).kickable) return msg.channel.send(` Bu kişinin susturmasını kaldıramıyorum çünkü \`benden daha yüksek bir role sahip\` ya da \`bana gerekli yetkileri vermedin\`.`);
		if (!mL.includes(user.id)) return msg.channel.send(` Bu kişi susturulmamış.`);
