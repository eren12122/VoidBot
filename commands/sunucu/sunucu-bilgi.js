...
module.exports = class ServerCommand extends Command {
    ...
    run(msg) {
        ...
        const embed = new RichEmbed()
            ...
            .addField(`❯ Üye Sayısı:`, `👤Toplam: ${msg.guild.members.size} \nÇevrimiçi: ${msg.guild.members.filter(m => m.user.presence.status === "online").size} \nRahatsız Etmeyin: ${msg.guild.members.filter(m => m.user.presence.status === "dnd").size} \nBoşta: ${msg.guild.members.filter(m => m.user.presence.status === "idle").size} \nÇevrımdışı/Görünmez: ${msg.guild.members.filter(m => m.user.presence.status === "offline").size} \nBot Sayısı: ${msg.guild.members.filter(m => m.user.bot).size}`, true)
